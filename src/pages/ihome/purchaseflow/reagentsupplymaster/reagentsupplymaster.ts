...
@IonicPage()
@Component({
  selector: "page-reagentsupplymaster",
  templateUrl: "reagentsupplymaster.html"
})
export class ReagentsupplymasterPage {
  userinfor: any = {};
  opinionList: any;
  httpurl: any;
  editingEntry: any = {};
  ...
  async init() {
    ...
    this.http
      .post(ENV.httpurlscm + handleNewUri5 + "/" + dataId, {})
      .subscribe(async data => {
        await this.requestOrg(
          handleUri5,
          dataId,
          data,
          this.navParams.get("bl")
        );
      });
  }
  requestOrg(uri, params, entity, bl) {
    let that = this;
    this.http
      .post(ENV.httpurlscm + uri + "/" + params, {})
      .timeout(9000)
      .subscribe(async data => {
  ...
  complex(entry) {
    var self = this;
    var _bizParams =
      entry.bizMap && entry.bizMap.bizStatus == "create"
        ? {
            params: {
              status: "0"
            }
        ...
        : {};
    self
      .request(ENV.httpurlscm + temporaryUri5, self.bizToFolwTitle(entry))
      // self.request(temporaryUri, self.bizToFolwTitle(entry))
      .then(
        data => {
          this.hjenty = data;
  ...
  getButton(pointId) {
    ...
    return new Promise((resolve, reject) => {
      self.http
        .post(ENV.httpurlscm + readButtonUri + "/" + pointId, {})
        .subscribe(
          resp => {
            self.buttons = resp;
    ...
    this.http
      .post(
        ENV.httpurlscm +
          readPointInfoUri +
          "/" +
          this.hjenty.flowSaveParDto.flowInstId,
        {}
      )
      .subscribe(
        async resp => {
  ...
  getDefaultOpinion() {
    ...
    if (
      self.hjenty.flowSaveParDto.flowInstName === "采购经办人" &&
      !self.hjenty.bizEntity.caCode
    ) {
      self.OPINION_TITLE =
        "同意，建议进行【" +
        purchaseWayStatuss[self.hjenty.bizEntity.purchaseWay] +
        "】";
    }
  }
  // 公共的发送请求的方法
  request(uri, params) {
    return new Promise((resolve, reject) => {
      return this.http.post(uri, params).subscribe(
        resp => {
          resolve(resp);
        },
        error => {
  ...
  getConInfomations() {
    ...
    this.http
      .post(
        ENV.httpurlscm + this.contractDetailReadUri,
        this.getParams(),
        params
      )
      .subscribe(data => {
  ...
  getDeliveryPlanInfos() {
    ...
    this.http
      .post(ENV.httpurlscm + this.deliveryReadUri, this.getParams(), params)
      .subscribe(data => {
  ...
  getpayInformations() {
    ...
    this.http
      .post(ENV.httpurlscm + this.payReadUri, this.getParams(), params)
      .subscribe(data => {
  ...
  getDataInformations() {
    ...
    this.http
      .post(ENV.httpurlscm + this.readSubUri, this.getParams(), parmas)
      .subscribe(data => {
  ...
  getOpions() {
    ...
    self.http
      ...
      .subscribe(resp => {
        ...
        _.map(self.purchaseTypeList, d => {
          if (
            d.optionValue === self.editingEntry.bizEntity.technologyAgreement
          ) {
            self.editingEntry.bizEntity.technologyAgreementName = d.optionName;
          }
          if (d.optionValue === self.editingEntry.bizEntity.contractType) {
            self.editingEntry.bizEntity.contractTypeName = d.optionName;
          }
        });
      });
    ...
    self.http
      ...
      .subscribe(resp => {
        _.map(resp, d => {
          if (
            d["optionValue"] === self.editingEntry.bizEntity.informationSource
          ) {
      ...
      .subscribe(resp => {
        _.map(resp, d => {
          if (
            ...
            self.editingEntry.bizEntity.technologyAgreementName =
              d["optionName"];
          }
        });
      });
  ...
  currAmount(entry) {
    ...
    if (!!entry && !!entry.flowSaveParDto.flowInstId) {
      ...
      self.http
        .post(ENV.httpurlscm + getAmountConfigUri, {})
        .subscribe(resp => {
          var amounts = resp;
          ...
          if (Number(amount) <= amounts["firstAmount"]) {
            ...
            Number(amount) > amounts["firstAmount"] &&
            Number(amount) <= amounts["secondAmount"]
          ) {
            self.currAmounts = {
            ...
            Number(amount) > amounts["secondAmount"] &&
            Number(amount) <= amounts["thirdAmount"]
          ) {
            self.currAmounts = {
            ...
            Number(amount) > amounts["thirdAmount"] &&
            Number(amount) <= amounts["fourthAmount"]
          ) {
            self.currAmounts = {
  ...
  transformEntry(entry) {
    ...
    if (
      (!entry || !entry.flowSaveParDto.flowInstId) &&
      entry.bizEntity.status != "draft"
    ) {
    ...
    */
    ...
    if (self.isButton) {
      self.isButton = _.includes(
        self.editingEntry.flowSaveParDto.flowInstName || "",
        "合同管理"
      )
        ? true
        : self.isButton;
    ...
    self.globalData
      .transformEntity(
        ...
        valueFields1,
        destFields1,
        [self.editingEntry.bizEntity],
        "cgIp"
      )
      .then(data => {
        self.editingEntry.bizEntity =
          data["length"] > 0 ? data[0] : self.editingEntry.bizEntity;
        let inputField2 = "orgCode";
        let valueFields2 = ["deptName"];
        let destFields2 = ["orgCodeName"];
        self.globalData.transformDeptEntity(
          inputField2,
          valueFields2,
          destFields2,
          [self.editingEntry.bizEntity],
    ...
    self.globalData.transformStaffEntity(
      inputField,
      valueFields,
      destFields,
      [self.editingEntry.bizEntity],
    ...
    self.contractDetailReadUri = detailReadUri + "/" + entry.bizEntity.id; // 根据合同主键id查询合同明细uri
    ...
    if (self.editingEntry.bizEntity.chargeBusinessUserID) {
      let userIds = self.editingEntry.bizEntity.chargeBusinessUserID.split(",");
      let userIdEntry = _.map(userIds, id => {
        return { userId: id };
      });
      let inputField = "userId";
      ...
      self.globalData
        ...
        .then(data => {
          self.fgUserName = _.map(data, e => e["userName"]).join(",");
        });
    }
  }
  //获取附件信息
  getFileList() {
    this.http
      .get(ENV.httpurlscm + fileSearchUri + this.editingEntry.bizEntity.id)
      .subscribe(data => {
        this.fileList = data;
      });
  }
  ...
  getOpinionList() {
    ...
    this.http
      .post(ENV.httpurlscm + readFlowOpinionUri + "/" + entity.flowInstId, {})
      .subscribe(
        resp => {
  ...
  viewFlow() {
    this.navCtrl.push("FlowprocessPage", {
      entry: this.navParams.get("entry")
    });
  }
  //选人
  selectStaffType(type) {
    ...
    if (type === "cdUserName") {
      ...
      uri = getDirectDeptStaffsUri + self.userinfor.orgId;
    } else if (type === "chargeBusinessUserName") {
      uri = roleStaffUri + roleIdTwo;
    }
    let profileModal = this.modalCtrl.create("PagingSelectPage", {
      requesttype: "get",
      type: null,
      url: uri,
      bizdata: null
    ...
    profileModal.onDidDismiss(selectedItem => {
      if (selectedItem) {
        if (type === "cdUserName") {
          ...
          self.editingEntry.bizEntity.contractDraftingStaff =
            selectedItem.userId;
        } else if (type === "chargeBusinessUserName") {
          self.fgUserName = selectedItem.userName;
          self.editingEntry.bizEntity.chargeBusinessUserID =
            selectedItem.userId;
        }
      }
      //做一次保存操作，用于保存选择的人员
  ...
  saveBizData(entity) {
    return new Promise((resolve, reject) => {
      return this.http.post(ENV.httpurlscm + createUri5, entity).subscribe(
        resp => {
          resolve(resp);
        },
        error => {}
