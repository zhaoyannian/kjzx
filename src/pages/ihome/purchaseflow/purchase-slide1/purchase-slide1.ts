...
@Component({
  selector: 'page-purchase-slide1',
  templateUrl: 'purchase-slide1.html',
})
export class PurchaseSlide1Page {
  //当前选择的分组按钮
  toggle = 'awiatData';
  //第几页
  page: any = 1;
  ...
  ngOnInit() {
    ...
    this.events.subscribe('purchase:awiatData', (type, num, time) => {
      this.toggle = num;
      this.flowType = type;
  ...
  tabslideNextRefreshFn(infiniteScroll) {
    if (this.totalCount == this.allList.length) {
      ...
      this.http.post(ENV.httpurlscm + this.url, this.getParams(), { params: pageInfo, observe: "response" }).timeout(9000).subscribe(data => {
        ...
        } else if (this.flowType == 2 || this.flowType == 6) {
