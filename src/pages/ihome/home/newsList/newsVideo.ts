...
@Component({
  ...
  templateUrl: 'newsVideo.html',
})
export class NewsVideoPage {
  ...
  constructor(public navCtrl: NavController, public nat: NativeService, public navParams: NavParams,public http:HttpClient, public g: globalData) {
    ...
    // this.menuId=str.substring(str.indexOf("=")+1,str.indexOf("&"));
    // if(str.indexOf("&")<=0){
    //   this.menuId=str.substring(str.indexOf("=")+1);
    //   }
    //   this.init();
    this.menuId = str;
  ...
  init(refresher, infiniteScroll){
    ...
    let that = this;
    that.http.post(ENV.httpurl+"/api/publArticle/queryilApp", pa, { params: that.pageInfo }).subscribe(async (resp)=>{
  ...
  viewNewsDetialFn(obj){
    this.navCtrl.push(NewsDetailPage,{entity:obj,type:'video',title:this.newTitle});
