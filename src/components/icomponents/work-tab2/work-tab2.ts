...
import { queryAllAliasUri, queryListUri, queryListUri2, queryListUri3, queryListUri4, queryAllAliasUri4, queryListUri5, queryListUri6, queryListUri7, queryListUri8, queryListUri9, queryListUri10,queryListByPageUri3, deleteUri3, queryAllAliasUri3,sealSearchyb,icardSearchyb,networkSearchyb,netSearchyb ,myNotifyUri} from './../../../icommon/provider/Constants';
...
@Component({
  selector: 'work-tab2',
  templateUrl: 'work-tab2.html'
})
export class WorkTab2Component {
  @Input() aliasMold;
  //第几页
  page: any = 1;
  //一页多少条
  ...
  queryKey: any = '';
  myNotifyUri:any;
  constructor(public globalData: globalData, public modalCtrl: ModalController, public nativeService: NativeService, public NavCtrl: NavController, public navParams: NavParams, public http: HttpClient, private events: Events, ) {
  ...
  ngOnInit() {
    this.events.subscribe('tabs:complateData', (num, time) => {
      this.toggle = num;
      this.queryKey = time;
      this.page = 1;
    ...
    if (this.aliasMold == 'ProjectDaily') {
      ...
      if(this.queryKey.trim() != ''){
        this.myNotifyUri = icardSearchyb;
      }else{
        this.myNotifyUri = myNotifyUri;
      }  
      // 数据字典翻译
      ...
      if(this.queryKey.trim() != ''){
        this.myNotifyUri = networkSearchyb;
      }else{
        this.myNotifyUri = myNotifyUri;
      } 
  ...
  commonLoad(data, pageInfo, refresher, infiniteScroll) {
    let that = this;
    that.getMyNotify(pageInfo, that.aliasMold, true, {}).then(result => {
